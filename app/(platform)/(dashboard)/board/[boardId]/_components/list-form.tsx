...
export const ListForm = ({ boardId }: { boardId: string }) => {
  ...
  if (isEditing) {
    return (
      <ListWrapper>
        <form
          className="w-full p-3 my-2 rounded-md bg-white space-y-4 shadow-md  relative"
          ref={formRef}
          action={handleSubmit}
        >
          <FormInput
  ...
  return (
    <ListWrapper>
      <button
        ...
        className="w-full py-3.5 md:m-4 my-2  rounded-md bg-white/80 hover:bg-white/50  transition p-3 flex items-center font-medium text-sm"
      >
        <Plus />
        Add a list
      </button>
